...
\paragraph{Alternative Prioritizations.}  The above scheme has the
...
mutants that change code near untaken branches are given higher
priority.  Mutants of code that is well-covered, on the other hand, or
that are deep within completely-uncovered code,
are lowered in priority.  If we imagine the example proposed in the
...
prioritzation may not be as obviously good as it seems at first.
Imagine that the {\tt hard1} condition is indeed on the coverage
frontier, but that a large amount of additional code is present after
the branch is taken but before the {\tt return 0} statement.  The {\tt
